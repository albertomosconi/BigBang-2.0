function buildOrdersList(orders) {
    ...
    orders.map(order => {
        ...
        idDateText.textContent = "ordered on " + date[0] + " at " + date[1] + " - ID: " + order.id; 
        ...
        let values = [order.total_items_cost, order.shipping_cost, order.total_items_cost + order.shipping_cost];
        [...Array(3).keys()].map(i => {
        ...
        order.items.map(item => {
            let tr = document.createElement("tr")
            let values = [item.quantity+"x", item.details.name, item.cost];
            [...Array(3).keys()].map(i => {
